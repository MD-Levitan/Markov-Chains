...
def MLE_algorithm(sequence):
    ...
    P = np.array([[transition(sequence, i, j, s) for j in range(0, sequence.A)] for i in array_of_value])
...
def transition(sequence, array_i, j, s):
...
def estimation_model(sequence, model, alg='bt', params=[]):
    ...
    cmm = CMM_S(model.s, model.N, model.Pi, P)
    return cmm
